...
(function(process) {
  ...
  function invokeEventListeners(listeners, target, eventImpl) {
    ...
    for (let i = 0; i < handlers.length; ++i) {
      ...
      if (listeners.indexOf(listener) === -1 || (eventImpl.eventPhase === Event.CAPTURING_PHASE && !listener.capture) || (eventImpl.eventPhase === Event.BUBBLING_PHASE && listener.capture)) {
        continue;
      }
      try {
