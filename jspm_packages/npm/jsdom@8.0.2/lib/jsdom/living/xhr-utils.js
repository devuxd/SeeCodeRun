...
(function(Buffer, process) {
  ...
  exports.createClient = function createClient(xhr, callback) {
    ...
    const hasBody = body !== undefined && body !== null && body !== "" && !(flag.method === "HEAD" || flag.method === "GET");
    if (hasBody && !flag.formData) {
      options.body = body;
    }
    try {
      const client = request(options).on("response", (response) => callback(null, response)).on("error", callback);
      if (hasBody && flag.formData) {
