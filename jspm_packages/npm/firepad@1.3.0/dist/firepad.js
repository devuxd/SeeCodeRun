  ...
  (function(name, definition, context) {
      ...
      RichTextToolbar.prototype.makeDropdown_ = function(title, eventName, items, value_suffix) {
        ...
        function showDropdown() {
          if (!isShown) {
            ...
            utils.on(document, 'click', hideDropdown, true);
            isShown = true;
          }
        }
        ...
        function hideDropdown() {
          if (isShown) {
            list.style.display = '';
            utils.off(document, 'click', hideDropdown, true);
            isShown = false;
          }
          justDismissed = true;
          setTimeout(function() {
    ...
    firepad.RichTextCodeMirror = (function() {
      ...
      RichTextCodeMirror.prototype.updateLineAttributes = function(startLine, endLine, updateFn) {
        for (var line = startLine; line <= endLine; line++) {
          ...
          if (text[0] !== LineSentinelCharacter) {
            ...
            this.updateTextAttributes(lineStartIndex, lineStartIndex + 1, updateFn, null, true);
          }
        }
      };
      ...
      RichTextCodeMirror.prototype.makeOrderedListElement_ = function(number) {
        return utils.elt('div', number + '.', {'class': 'firepad-list-left'});
      };
      RichTextCodeMirror.prototype.makeUnorderedListElement_ = function() {
        return utils.elt('div', '\u2022', {'class': 'firepad-list-left'});
      };
    ...
    firepad.RichTextCodeMirrorAdapter = (function() {
      ...
      RichTextCodeMirrorAdapter.prototype.setOtherCursor = function(cursor, color, clientId) {
        ...
        if (cursor.position === cursor.selectionEnd) {
          ...
          var rule = '.' + selectionClassName + ' {' + ' background: ' + hex2rgb(color) + ';\n' + ' background: ' + hex2rgb(color, transparency) + ';' + '}';
          ...
          return this.cm.markText(fromPos, toPos, {className: selectionClassName});
        }
      };
        ...
        for (var i = 0; i < styles.length; i++) {
          ...
          switch (prop) {
            ...
            case 'font-size':
              ...
              break;
            case 'font-family':
              var font = firepad.utils.trim(val.split(',')[0]);
              font = font.replace(/['"]/g, '');
        ...
        while (op) {
          ...
          if (newLine) {
            ...
            while (listTypeStack.length < indent) {
              var toOpen = listType || LIST_TYPE.UNORDERED;
