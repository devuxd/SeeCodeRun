...
export function toAst(source, options) {
  if (options) {
...
function getFirstNode(jPath, j) {
  return j(jPath).get('body', 0).node;
}

export function wrapCallExpressions(ast, locationMap) {
  const paths = [];
  let wrapped = ast
  ...
  for (const i in paths) {
    ...
    j(path).replaceWith(p => {
        ...
        locationMap.push({type: l.FunctionCall, expressionType: j.ExpressionStatement.name, loc: {...path.value.loc}});
...
export function wrapFunctionExpressions(ast, locationMap) {
  ...
  for (const i in paths) {
    ...
    let id = locationMap.length;
    locationMap.push({type: l.FunctionStart, expressionType: j.ExpressionStatement.name, loc: {...path.value.loc}});
    ...
    if (body.length && body[body.length - 1].type !== 'ReturnStatement') {
      ...
      locationMap.push({type: l.FunctionEnd, expressionType: body[body.length - 1].type, loc: {...path.value.loc}});
    ...
    j(path).find(j.ReturnStatement).replaceWith(p => {
        id = locationMap.length;
        locationMap.push({type: l.FunctionEnd, expressionType: body[body.length - 1].type, loc: {...p.value.loc}});
