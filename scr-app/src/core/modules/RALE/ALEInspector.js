...
export const ObjectPreview = (withStyles(styles)(
   (props) => {
      ...
      if (Array.isArray(object)) {
         const maxProperties = arrayMaxProperties ?? styles.arrayMaxProperties;
         ...
         const previewArray = disableListGroup ? makePreviewArray(
            ...
            (element, index) => (
               isMaxPreviewDepth ? <ObjectValue key={index} object={element}/>
                  : <ObjectPreview key={index} {...props} data={element}
...
const tableOn = (
   <TableChartIcon style={tableChartIconStyleOn}/>
);

const tableOff = (
   <TableChartOutlinedIcon
      style={tableChartIconStyleOff}/>
);

const tooltipTableOn = (
   <TableChartIcon
      style={{...tableChartIconStyleOn, marginTop: 0, marginRight: 0}}/>
);

const tooltipTableOff = (
   <TableChartOutlinedIcon
      style={{...tableChartIconStyleOff, marginTop: 0, marginRight: 0}}/>
);
...
const ALEInspector = (
   ...
   const onOverflow = useCallback((overflowed) => {
   ...
   const makeNodeRenderer = useCallback(
      (isCompact) => ({depth, name, data: _data, isNonenumerable}) => {
         ...
         const _aleObject = depth === 0 ? aleObject : aleObject?.getLiveRefOfDomLiveRef?.(object);
   ...
   const {inspector, tooltipInspector, disableInteractive} =
      useMemo(
         () => {
         ...
         [
            Inspector, inline, graphical, expressionId, isStrictLiteral, data,
            nodeRenderer, tooltipNodeRenderer, overflowed, isTable, isSupportTable
         ]
      );
   
   
   //console.log("LiveExpression", data, expressionId, expressionType, isOutput, outputRefs, logValue.live);
   ...
   const tooltipTableHandle = isSupportTable &&
      <div
         onClick={handleToggleTable}>{isTable ? tooltipTableOn : tooltipTableOff}
      </div>;
