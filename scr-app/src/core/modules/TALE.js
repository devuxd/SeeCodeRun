...
class Trace {
    ...
    static getReplacer = (
        ...
        const windowRoots = getWindowRoots?.() ?? {};
        return (key, value) => {

            const xme = GraphicalIdiom.isNode(value);
            ...
            if (i > -1) {
                return JSEN.replacer(
                    null, `${MAGIC_TAG}${Object.keys(windowRoots)[i]}`
                );
            }

            if (scrObject && scrObject === value) {
                return undefined;
            }
            ...
            if (
                ...
                domNodes.push(value);
                return JSEN.replacer(null, `${MAGIC_TAG}${domNodes.length - 1}`);
            }

            const apiName = GraphicalIdiom.isNode(value);
            ...
            switch (apiName) {
                case SupportedApis.jQuery:
                    ...
                    for (let i = 0; i < value.length; i++) {
                        const domNode = value[i];
                        const val = copifyDOMNode(domNode);
                        const domId = domNodes.indexOf(domNode);

                        nodes.push(domNode);
                        values.push(val);

                        if (domId >= 0) {
                            // todo: multipli live ref
                            val.liveRef = `${MAGIC_TAG}${domId}`;
                        } else {
                            // console.log('ref', value.tagName,res, value);
                            domNodes.push(domNode);
                            val.liveRef = `${MAGIC_TAG}${domNodes.length - 1}`;
                            // Create an observer instance linked to the callback function
                            const observer = new MutationObserver(obsCallback);
                            // Start observing the target node for configured mutations
                            observer.observe(domNode, obsConfig);
                            domNodesObs.push(observer);
                        }
                    ...
                    res.isDOM = true;
                    res.outputRefs ??= [];
                    res.outputRefs.push(...nodes);
                    onDomNodeAdded();
                    return JSEN.replacer(null, values);
                ...
                case SupportedApis.standard:
                    ...
                    const domId = domNodes.indexOf(value);
                    if (domId >= 0) {
                        val.liveRef = `${MAGIC_TAG}${domId}`;
                    } else {
                        // console.log('ref', value.tagName,res, value);
    ...
    pushEntry = (pre, value, post, type, refId) => {
        ...
        const apiName = GraphicalIdiom.isNode(value);
        const data = JSEN.stringify(value, Trace.getReplacer(
            res,
            this.getWindowRoots,
            this.domNodes,
            this.domNodesObs,
