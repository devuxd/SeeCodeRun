...
class NotificationCenter extends PureComponent {
   ...
   componentDidUpdate() {
      ...
      if (isSnackbarOpen) {
         ...
         const id = enqueueSnackbar(getNetworkStateMessage(), options);
         
         clearTimeout(this.closeSnackbarTid);
         if (isSuccess) {
            this.closeSnackbarTid = setTimeout(() => {
               try {
                  closeSnackbar(id);
               } catch (e) {
                  console.log('closeSnackbar', e);
               }
   ...
   componentWillUnmount() {
      clearTimeout(this.closeSnackbarTid);
      clearTimeout(this.state.disconnectedTimeout);
   }
}
