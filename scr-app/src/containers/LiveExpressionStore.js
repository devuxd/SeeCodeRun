...
class LiveExpressionStore extends PureComponent {
   ...
   componentDidUpdate(prevProps/*, prevState, snapshot*/) {
      ...
      const {
         autorunDelay = 0, firecoPad, editorsTexts, updateBundle, aleContext
      } = this.props;
      const timeline = aleContext?.aleInstance?.scr?.timeline;
      //todo: logs!
      if (timeline && this.prevTimeline !== timeline) {
         this.prevTimeline = this.timeline;
         this.isRefresh = true;
         // console.log('aleContext', aleContext, this.prevTimelineLength);
         this.handleTraceChange({timeline, logs: [], mainLoadedTimelineI: 100});
      }
   ...
   refreshTimeline = () => {
      ...
      // console.log("refreshTimeline", this.isRefresh, this.timeline);
      if (this.isRefresh) {
         if (this.didUpdate) {
            ...
            this.props.liveExpressionStoreChange?.(this, true);//set via props
         }
      } else {
         const timeline = this.timeline;
         if (this.prevTimelineLength !== timeline?.length) {
            this.prevTimelineLength = timeline.length;
            this.props.liveExpressionStoreChange?.(this, false);//set via props
         }
      }
   };
}
